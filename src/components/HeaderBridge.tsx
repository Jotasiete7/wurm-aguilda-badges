...
import localHeaderStyles from './Header.module.css';
import agHeaderStyles from '@ecossistema-guilda/layout/Header.module.css';
...
export const HeaderBridge: React.FC<HeaderBridgeProps> = ({ session, isAdmin, logoutAction }) => {
    ...
    const authData = {
        ...
        loginButton: !session?.user ? (
            <Link href="/login" className="btn btn-primary flex items-center gap-2" style={{ backgroundColor: '#5865F2', borderColor: '#5865F2' }}>
    ...
    const languageSwitch = (
        <LanguageSwitch 
            ...
            styles={localHeaderStyles} // We will use the bridge to pass styles if needed, but actually the Header has its own styles
        />
    );
